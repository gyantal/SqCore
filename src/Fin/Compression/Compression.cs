...
namespace QuantConnect
{
    ...
    public static class Compression
    {
            ...
            try
            {
                using (var zipStream = new ZipOutputStream(File.Create(destination)))
                {
                    ...
                    foreach (var file in files)
                    {
                        if (!File.Exists(file))
                        {
                            Utils.Logger.Trace($"ZipFiles(): File does not exist: {file}");
                            continue;
                        }
        ...
        public static StreamReader Unzip(string filename, string zipEntryName, out ZipFile zip)
        {
            ...
            try
            {
                if (File.Exists(filename))
                {
                    ...
                    {
                        Utils.Logger.Error(err, "Inner try/catch");
                ...
                {
                    Utils.Logger.Error($"Data.UnZip(2): File doesn\'t exist: {filename}");
                }
            }
            catch (Exception err)
            {
                Utils.Logger.Error(err, "File: " + filename);
            }
            return reader;
        }
        ...
        public static IEnumerable<KeyValuePair<string, List<string>>> Unzip(string filename)
        {
            if (!File.Exists(filename))
            {
                Utils.Logger.Error($"Compression.Unzip(): File does not exist: {filename}");
        ...
        public static List<string> ReadLines(string filename)
        {
            if (!File.Exists(filename))
            {
                Utils.Logger.Error($"Compression.ReadFirstZipEntry(): File does not exist: {filename}");
        ...
        public static List<string> UnzipToFolder(string zipFile)
        {
            ...
            catch
            {
                ...
                Utils.Logger.Error($"Compression.UnzipToFolder(): Failure: zipFile: {zipFile} - outFolder: {outFolder} - files: {string.Join(",", files)}");
                throw;
            }
            finally
            {
