    ...
    public static class Extensions
    {
        ...
        public static string DownloadData(this string url, Dictionary<string, string> headers = null)
        {
            using (var client = new HttpClient())
            {
                ...
                {
                    Utils.Logger.Error(ex, $"DownloadData(): failed for: '{url}'");
                    return null;
                }
            }
        }
        ...
        public static byte[] DownloadByteArray(this string url)
        {
            using (var wc = new HttpClient())
            {
                ...
                {
                    Utils.Logger.Error(ex, $"DownloadByteArray(): failed for: '{url}'");
                    return null;
                }
            }
        }
        ...
        public static MemoryStream GetMemoryStream(Guid guid)
        {
            ...
            throw new NotImplementedException(); // commented out, because of Microsoft.IO dependency. It seems it is only used in Tests.
        }
        ...
        public static void StopSafely(this Thread thread, TimeSpan timeout, CancellationTokenSource token = null)
        {
            if (thread != null)
            {
                try
                {
                    ...
                    Utils.Logger.Trace($"StopSafely(): waiting for '{thread.Name}' thread to stop...");
                    // just in case we add a time out
                    if (!thread.Join(timeout))
                    {
                        Utils.Logger.Error($"StopSafely(): Timeout waiting for '{thread.Name}' thread to stop");
                    }
                }
        ...
        public static bool TryParseSecurityType(this string value, out SecurityType securityType, bool ignoreCase = true)
        {
            ...
            {
                Utils.Logger.Error($"Extensions.TryParseSecurityType(): Attempted to parse unknown SecurityType: {value}");
            }
        ...
        {
            Utils.Logger.Error(exception, $"Extensions.SetRuntimeError(): RuntimeError at {algorithm.UtcTime} UTC. Context: {context}");
