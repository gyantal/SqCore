    ...
    public class Cash
    {
            ...
            )
        {
                    ...
                    Symbol == x.Value.QuoteCurrency))
            {
                ...
                Utils.Logger.Error($"No tradeable pair was found for currency {Symbol}, conversion rate to account currency ({accountCurrency}) will be set to zero.");
                CurrencyConversion = null;
                ConversionRate = 0m;
                return null;
            }
            ...
            var makeNewSecurity = new Func<Symbol, Security>(symbol =>
            {
                ...
                Utils.Logger.Trace($"Cash.EnsureCurrencyDataFeed(): Adding {symbol.Value} for cash {Symbol} currency feed");
