...
namespace QuantConnect.Python
{
    ...
    public static class PythonInitializer
    {
        ...
        public static void Initialize(bool beginAllowThreads = true)
        {
            if (!_isInitialized)
            {
                Utils.Logger.Trace("PythonInitializer.Initialize(): start...");
                ...
                Utils.Logger.Trace("PythonInitializer.Initialize(): ended");
            }
        }
        ...
        public static void Shutdown()
        {
            if (_isInitialized)
            {
                Utils.Logger.Trace("PythonInitializer.Shutdown(): start");
                ...
                Utils.Logger.Trace("PythonInitializer.Shutdown(): ended");
            }
        }
        ...
        public static void AddAlgorithmLocationPath(string algorithmLocation)
        {
            ...
            {
                Utils.Logger.Error($"PythonInitializer.AddAlgorithmLocationPath(): Unable to find algorithm location path: {algorithmLocation}.");
                return;
            }
        ...
        public static bool ActivatePythonVirtualEnvironment(string pathToVirtualEnv)
        {
            ...
            {
                Utils.Logger.Error($"PythonIntializer.ActivatePythonVirtualEnvironment(): Path {pathToVirtualEnv} to virtual environment does not exist");
                return false;
            }
            ...
            if(!includeSystemPackages.HasValue)
            {
                ...
                Utils.Logger.Error($"PythonIntializer.ActivatePythonVirtualEnvironment(): virtual env '{pathToVirtualEnv}'. Failed to find system packages configuration. ConfigFile.Exits: {configFile.Exists}. Will default to true.");
            }
            else
            {
                Utils.Logger.Trace($"PythonIntializer.ActivatePythonVirtualEnvironment(): virtual env '{pathToVirtualEnv}'. Will use system packages: {includeSystemPackages.Value}");
            }
        ...
        private static void TryInitPythonVirtualEnvironment()
        {
            ...
            using (Py.GIL())
            {
                ...
                if (Utils.Logger.DebuggingEnabled())
                {
                    ...
                    Utils.Logger.Debug($"PythonIntializer.InitPythonVirtualEnvironment(): PYTHONHOME: {os.getenv("PYTHONHOME")}." +
        ...
        private static void ConfigurePythonPaths()
        {
            ...
            AddPythonPaths(pythonAdditionalPaths.Where(path =>
            {
                ...
                if (!pathExists)
                {
                    Utils.Logger.Error($"PythonInitializer.ConfigurePythonPaths(): Unable to find python path: {path}. Skipping.");
                }
