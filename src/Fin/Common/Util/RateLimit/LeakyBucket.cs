        ...
        public bool TryConsume(long tokens)
        {
            ...
            lock (_sync)
            {
                ...
                if (tokens > _available)
                {
                    ...
                    Utils.Logger.Trace($"LeakyBucket.TryConsume({tokens}): Failed to consumed tokens. Available: {_available}");
                    return false;
                }

                // subtract the number of tokens consumed
                _available = _available - tokens;
                Utils.Logger.Trace($"LeakyBucket.TryConsume({tokens}): Successfully consumed tokens. Available: {_available}");
                return true;
            }
        }
