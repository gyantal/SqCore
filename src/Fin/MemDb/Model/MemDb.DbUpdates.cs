...
public partial class MemDb
{
    ...
    public string AddOrEditPortfolio(int p_id, User? p_user, string p_name, int p_parentFldId, string p_currency, string p_prtfType,  string p_algorithm, string p_algorithmParam, string p_userAccess, string p_note, out Portfolio? p_newItem)
    {
        Portfolio? prtf = null;
        string errMsg = String.Empty;
        ...
        {
            string creationTime = DateTime.UtcNow.TohYYYYMMDDHHMMSS(); // DateTime.Now.ToString() => "CTime":"2022-10-13T20:00:00"
            prtf = m_memData.AddNewPortfolio(p_user, p_name, p_parentFldId, creationTime, AssetHelper.gStrToCurrency[p_currency], AssetHelper.gStrToPortfolioType[p_prtfType], p_algorithm, p_algorithmParam, AssetHelper.gStrToSharedAccess[p_userAccess], p_note, sharedUsersWith);
            if (prtf == null)
                errMsg = "Cannot create new item.";
            else
            {
