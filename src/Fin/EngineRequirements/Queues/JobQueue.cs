...
namespace QuantConnect.Queues
{
    ...
    public class JobQueue : IJobQueueHandler
    {
        ...
        public AlgorithmNodePacket NextJob(out string location)
        {
            ...
            Utils.Logger.Trace($"JobQueue.NextJob(): Selected {location}");
            ...
            if (_liveMode)
            {
                ...
                {
                    Utils.Logger.Error(err, $"Error resolving BrokerageData for live job for brokerage {liveJob.Brokerage}");
                }
                ...
                foreach (var dataHandlerName in dataHandlers.DeserializeList())
                {
                    ...
                    {
                        Utils.Logger.Trace($"JobQueue.NextJob(): Not able to fetch data handler factory with name: {dataHandlerName}");
                        continue;
                    }
