    ...
    public class AlgorithmPythonWrapper : IAlgorithm
    {
        ...
        public AlgorithmPythonWrapper(string moduleName)
        {
            try
            {
                using (Py.GIL())
                {
                    Utils.Logger.Trace($"AlgorithmPythonWrapper(): Python version {PythonEngine.Version}: Importing python module {moduleName}");

                    var module = Py.Import(moduleName);

                    Utils.Logger.Trace($"AlgorithmPythonWrapper(): {moduleName} successfully imported.");
                    ...
                    foreach (var name in pyList)
                    {
                        ...
                        {
                            Utils.Logger.Trace("AlgorithmPythonWrapper(): Creating IAlgorithm instance.");
