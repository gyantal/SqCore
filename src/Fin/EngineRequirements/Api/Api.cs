...
namespace QuantConnect.Api
{
    ...
    public class Api : IApi, IDownloadProvider
    {
        ...
        public bool DownloadData(string filePath, string organizationId)
        {
            ...
            if (!dataLink.Success)
            {
                Utils.Logger.Trace($"Api.DownloadData(): Failed to get link for {filePath}. " +
            ...
            {
                Utils.Logger.Error($"Api.DownloadData(): Failed to download zip for path ({filePath})");
                return false;
            }
        ...
        public static string FormatPathForDataRequest(string filePath, string dataFolder = null)
        {
            if (filePath == null)
            {
                Utils.Logger.Error("Api.FormatPathForDataRequest(): Cannot format null string");
                return null;
            }
