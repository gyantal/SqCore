        ...
        where T : IBaseData
    {
        ...
        public override bool Update(IBaseData input)
        {
            ...
            if (_previousInput.TryGetValue(input.Symbol.ID, out _previousSymbolInput) && input.EndTime < _previousSymbolInput.EndTime)
            {
                ...
                Utils.Logger.Error($"This is a forward only indicator: {Name} Input: {input.EndTime:u} Previous: {_previousSymbolInput.EndTime:u}. It will not be updated with this input.");
                return IsReady;
            }
