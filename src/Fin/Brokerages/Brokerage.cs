...
namespace QuantConnect.Brokerages
{
    ...
    public abstract class Brokerage : IBrokerage
    {
        ...
        protected virtual void OnOptionPositionAssigned(OrderEvent e)
        {
            try
            {
                Utils.Logger.Debug("Brokerage.OptionPositionAssigned(): " + e);
        ...
        protected virtual void OnOptionNotification(OptionNotificationEventArgs e)
        {
            try
            {
                Utils.Logger.Debug("Brokerage.OnOptionNotification(): " + e);
        ...
        protected virtual void OnDelistingNotification(DelistingNotificationEventArgs e)
        {
            try
            {
                Utils.Logger.Debug("Brokerage.OnDelistingNotification(): " + e);
        ...
        protected virtual void OnAccountChanged(AccountEvent e)
        {
            try
            {
                Utils.Logger.Trace($"Brokerage.OnAccountChanged(): {e}");
        ...
        protected virtual void OnMessage(BrokerageMessageEvent e)
        {
            try
            {
                ...
                {
                    Utils.Logger.Error("Brokerage.OnMessage(): " + e);
                }
                else
                {
                    Utils.Logger.Trace("Brokerage.OnMessage(): " + e);
                }
        ...
        protected virtual List<Holding> GetAccountHoldings(Dictionary<string, string> brokerageData, IEnumerable<Security> securities)
        {
            ...
            {
                Utils.Logger.Debug("Brokerage.GetAccountHoldings(): starting...");
            }
            ...
            if (brokerageData != null && brokerageData.Remove("live-holdings", out var value) && !string.IsNullOrEmpty(value))
            {
                ...
                Utils.Logger.Trace($"Brokerage.GetAccountHoldings(): sourcing holdings from provided brokerage data, found {result.Count} entries");
                return result;
            }
        ...
        protected virtual List<CashAmount> GetCashBalance(Dictionary<string, string> brokerageData, CashBook cashBook)
        {
            if (Utils.Logger.DebuggingEnabled())
            {
                Utils.Logger.Debug("Brokerage.GetCashBalance(): starting...");
            }
            ...
            if (brokerageData != null && brokerageData.Remove("live-cash-balance", out var value) && !string.IsNullOrEmpty(value))
            {
                ...
                Utils.Logger.Trace($"Brokerage.GetCashBalance(): sourcing cash balance from provided brokerage data, found {result.Count} entries");
                return result;
            }
        ...
        public virtual bool PerformCashSync(IAlgorithm algorithm, DateTime currentTimeUtc, Func<TimeSpan> getTimeSinceLastFill)
        {
            try
            {
                ...
                {
                    Utils.Logger.Trace("Brokerage.PerformCashSync(): Reentrant call, cash sync not performed");
                    return false;
                }

                Utils.Logger.Trace("Brokerage.PerformCashSync(): Sync cash balance");
                ...
                {
                    Utils.Logger.Error(err, "Error in GetCashBalance:");
                }

                if (balances.Count == 0)
                {
                    Utils.Logger.Trace("Brokerage.PerformCashSync(): No cash balances available, cash sync not performed");
                    return false;
                }
                ...
                foreach (var balance in balances)
                {
                    ...
                    {
                        Utils.Logger.Trace($"Brokerage.PerformCashSync(): Unexpected cash found {balance.Currency} {balance.Amount}", true);
                ...
                foreach (var kvp in algorithm.Portfolio.CashBook)
                {
                    ...
                    if (balanceCash != default(CashAmount))
                    {
                        ...
                        if (Math.Abs(algorithm.Portfolio.CashBook.ConvertToAccountCurrency(delta, cash.Symbol)) > 5)
                        {
                            ...
                            Utils.Logger.Trace($"Brokerage.PerformCashSync(): {balanceCash.Currency} Delta: {delta:0.00}", true);
                        }
                    ...
                    else
                    {
                        ...
                        Utils.Logger.Trace($"Brokerage.PerformCashSync(): {cash.Symbol} was not found in brokerage cash balance, setting the amount to 0", true);
            ...
            Task.Delay(TimeSpan.FromSeconds(10)).ContinueWith(_ =>
                {
                    ...
                    if (getTimeSinceLastFill() <= TimeSpan.FromSeconds(20))
                    {
                        ...
                        Utils.Logger.Trace("Brokerage.PerformCashSync(): Unverified cash sync - resync required.");
                    }
                    else
                    {
                        Utils.Logger.Trace("Brokerage.PerformCashSync(): Verified cash sync.");
