...
namespace QuantConnect.Brokerages.Backtesting
{
    ...
    public class BacktestingBrokerage : Brokerage
    {
        ...
        public override bool PlaceOrder(Order order)
        {
            ...
            {
                Utils.Logger.Trace("BacktestingBrokerage.PlaceOrder(): Type: " + order.Type + " Symbol: " + order.Symbol.Value + " Quantity: " + order.Quantity);
            }
        ...
        public override bool UpdateOrder(Order order)
        {
            if (Algorithm.LiveMode)
            {
                Utils.Logger.Trace("BacktestingBrokerage.UpdateOrder(): Symbol: " + order.Symbol.Value + " Quantity: " + order.Quantity + " Status: " + order.Status);
            }
        ...
        public override bool CancelOrder(Order order)
        {
            if (Algorithm.LiveMode)
            {
                Utils.Logger.Trace("BacktestingBrokerage.CancelOrder(): Symbol: " + order.Symbol.Value + " Quantity: " + order.Quantity);
            }
        ...
        public virtual void Scan()
        {
            lock (_needsScanLock)
            {
                ...
                foreach (var kvp in _pending.OrderBySafe(x => x.Key))
                {
                    ...
                    if (order == null)
                    {
                        Utils.Logger.Error("BacktestingBrokerage.Scan(): Null pending order found: " + kvp.Key);
                    ...
                    {
                        Utils.Logger.Error($"BacktestingBrokerage.Scan(): Unable to process order: {order.Id}. The security no longer exists. UtcTime: {Algorithm.UtcTime}");
        ...
        public virtual void ActivateOptionAssignment(Option option, int quantity)
        {
            ...
            Utils.Logger.Trace($"BacktestingBrokerage.ActivateOptionAssignment(): OrderId: {ticket.OrderId}");
        }
        ...
        public void ProcessDelistings(Delistings delistings)
        {
            ...
            {
                Utils.Logger.Trace($"BacktestingBrokerage.ProcessDelistings(): Delisting {delisting.Type}: {delisting.Symbol.Value}, UtcTime: {Algorithm.UtcTime}, DelistingTime: {delisting.Time}");
                ...
                if (!Algorithm.IsWarmingUp)
                {
                    ...
                    {
                        Utils.Logger.Trace("AlgorithmManager.Run(): " + cancelledOrder);
                    }
                }
            }
        }
