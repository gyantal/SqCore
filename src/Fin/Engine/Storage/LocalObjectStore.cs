...
namespace QuantConnect.Lean.Engine.Storage
{
    ...
    public class LocalObjectStore : IObjectStore
    {
        ...
        public virtual void Initialize(int userId, int projectId, string userToken, Controls controls)
        {
            ...
            Utils.Logger.Trace($"LocalObjectStore.Initialize(): Storage Root: {new FileInfo(AlgorithmStorageRoot).FullName}. StorageFileCount {controls.StorageFileCount}. StorageLimit {BytesToMb(controls.StorageLimit)}MB");
        }
        ...
        public virtual void Dispose()
        {
            ...
            {
                Utils.Logger.Error(err, "Error deleting storage directory.");
            }
        }
        ...
        private void Persist()
        {
            ...
            lock (_persistLock)
            {
                ...
                {
                    Utils.Logger.Error("LocalObjectStore.Persist()", err);
                    OnErrorRaised(err);
                }
                finally
                {
        ...
        protected virtual bool PersistData()
        {
            ...
            {
                Utils.Logger.Error(err, "LocalObjectStore.PersistData()");
                OnErrorRaised(err);
                return false;
            }
        }
