    ...
    public class ApiDataProvider : BaseDownloaderDataProvider
    {
        ...
        public ApiDataProvider()
        {
            ...
            if (string.IsNullOrEmpty(_organizationId))
            {
                ...
                Utils.Logger.Trace($"ApiDataProvider(): Will use organization Id '{_organizationId}'.");
            }
            ...
            if (organization.DataAgreement.Signed)
            {
                ...
                Utils.Logger.Trace("ApiDataProvider(): Data Terms of Use has been signed. \r\n" +
            ...
            if (balance < _purchaseLimit)
            {
                ...
                {
                    Utils.Logger.Error("ApiDataProvider(): Purchase limit is greater than balance." +
            ...
            if (LeanData.TryParseSecurityType(filePath, out SecurityType securityType, out var market) && _unsupportedSecurityType.Contains(securityType))
            {
                ...
                if (securityType != SecurityType.Future || !IsAuxiliaryData(filePath))
                {
                    if (!_invalidSecurityTypeLog)
                    {
                        ...
                        Utils.Logger.Error($"ApiDataProvider(): does not support security types: {string.Join(", ", _unsupportedSecurityType)}");
                    }
                    return false;
                }
            }
        ...
        protected virtual bool DownloadData(string filePath)
        {
            ...
            {
                Utils.Logger.Debug($"ApiDataProvider.Fetch(): Attempting to get data from QuantConnect.com's data library for {filePath}.");
            }

            if (_api.DownloadData(filePath, _organizationId))
            {
                Utils.Logger.Trace($"ApiDataProvider.Fetch(): Successfully retrieved data for {filePath}.");
                return true;
            }
