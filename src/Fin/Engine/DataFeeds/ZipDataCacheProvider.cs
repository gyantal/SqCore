                ...
                try
                {
                    ...
                    else
                    {
                        ...
                        catch (Exception exception)
                        {
                            ...
                            {
                                Utils.Logger.Error("ZipDataCacheProvider.Fetch(): Corrupt zip file/entry: " + filename + "#" + entryName + " Error: " + exception);
                            }
                            else throw;
                        }
                    }
            ...
            lock (_zipFileCache)
            {
                ...
                lock (cachedZip)
                {
                    if (cachedZip.Disposed)
                    {
                        ...
                        {
                            Utils.Logger.Error($"ZipDataCacheProvider.Store(): unexpected cache state for {fileName}");
        ...
        private bool Cache(string filename, out CachedZipFile cachedZip)
        {
            ...
            if (dataStream != null)
            {
                ...
                catch (Exception exception)
                {
                    if (exception is ZipException || exception is ZlibException)
                    {
                        Utils.Logger.Error("ZipDataCacheProvider.Fetch(): Corrupt zip file/entry: " + filename + " Error: " + exception);
                    }
                    else throw;
                }
