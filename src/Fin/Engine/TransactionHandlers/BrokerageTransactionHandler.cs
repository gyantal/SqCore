...
namespace QuantConnect.Lean.Engine.TransactionHandlers
{
    ...
    public class BrokerageTransactionHandler : ITransactionHandler
    {
        ...
        public OrderTicket Process(OrderRequest request)
        {
            ...
            {
                Utils.Logger.Trace("BrokerageTransactionHandler.Process(): " + request);
        ...
        protected virtual void WaitForOrderSubmission(OrderTicket ticket)
        {
            ...
            {
                Utils.Logger.Error("BrokerageTransactionHandler.WaitForOrderSubmission(): " +
        ...
        public OrderTicket UpdateOrder(UpdateOrderRequest request)
        {
            ...
            try
            {
                ...
                if (order == null)
                {
                    ...
                    Utils.Logger.Error("BrokerageTransactionHandler.Update(): Cannot update a null order");
                ...
                else if (order.Status == OrderStatus.New)
                {
                    ...
                    Utils.Logger.Error("BrokerageTransactionHandler.Update(): Cannot update a pending submit order with status " + order.Status);
                ...
                else if (order.Status.IsClosed())
                {
                    ...
                    Utils.Logger.Error("BrokerageTransactionHandler.Update(): Cannot update closed order with status " + order.Status);
        ...
        public OrderTicket CancelOrder(CancelOrderRequest request)
        {
            ...
            if (!_completeOrderTickets.TryGetValue(request.OrderId, out ticket))
            {
                Utils.Logger.Error("BrokerageTransactionHandler.CancelOrder(): Unable to locate ticket for order.");
            ...
            try
            {
                ...
                if (order == null)
                {
                    Utils.Logger.Error("BrokerageTransactionHandler.CancelOrder(): Cannot find this id.");
                    request.SetResponse(OrderResponse.UnableToFindOrder(request));
                }
                else if (order.Status == OrderStatus.New)
                {
                    Utils.Logger.Error("BrokerageTransactionHandler.CancelOrder(): Cannot cancel order with status: " + order.Status);
                    request.SetResponse(OrderResponse.InvalidNewStatus(request, order));
                }
                else if (order.Status.IsClosed())
                {
                    Utils.Logger.Error("BrokerageTransactionHandler.CancelOrder(): Cannot cancel order already " + order.Status);
                    request.SetResponse(OrderResponse.InvalidStatus(request, order));
                }
                else if (_algorithm.IsWarmingUp)
                {
        ...
        protected void Run()
        {
            ...
            {
                Utils.Logger.Trace("BrokerageTransactionHandler.Run(): Ending Thread...");
                IsActive = false;
            }
        }
        ...
        public virtual void ProcessSynchronousEvents()
        {
            ...
            if (!_algorithm.LiveMode)
            {
                ...
                {
                    Utils.Logger.Error("BrokerageTransactionHandler.ProcessSynchronousEvents(): Timed out waiting for request queue to finish processing.");
                }
                return;
            }
            ...
            Utils.Logger.Debug("BrokerageTransactionHandler.ProcessSynchronousEvents(): Start removing old orders...");
            ...
            Utils.Logger.Debug($"BrokerageTransactionHandler.ProcessSynchronousEvents(): New order count {_completeOrders.Count}. Exit");
        }
        ...
        public void Exit()
        {
            ...
            if (_processingThread != null)
            {
                ...
                {
                    Utils.Logger.Error("BrokerageTransactionHandler.Exit(): Exceed timeout: " + (int)(timeout.TotalSeconds) + " seconds.");
                }
            }
        ...
        private OrderResponse HandleSubmitOrderRequest(SubmitOrderRequest request)
        {
            ...
            {
                Utils.Logger.Error("BrokerageTransactionHandler.HandleSubmitOrderRequest(): Unable to add new order, order not processed.");
            ...
            {
                Utils.Logger.Error("BrokerageTransactionHandler.HandleSubmitOrderRequest(): Unable to retrieve order ticket, order not processed.");
        ...
        private OrderResponse HandleUpdateOrderRequest(UpdateOrderRequest request)
        {
            ...
            {
                Utils.Logger.Error("BrokerageTransactionHandler.HandleUpdateOrderRequest(): Unable to update order with ID " + request.OrderId);
                return OrderResponse.UnableToFindOrder(request);
            }
        ...
        private OrderResponse HandleCancelOrderRequest(CancelOrderRequest request)
        {
            ...
            if (!_completeOrders.TryGetValue(request.OrderId, out order) || !_completeOrderTickets.TryGetValue(request.OrderId, out ticket))
            {
                Utils.Logger.Error("BrokerageTransactionHandler.HandleCancelOrderRequest(): Unable to cancel order with ID " + request.OrderId + ".");
        ...
        private void HandleOrderEvent(OrderEvent orderEvent)
        {
            lock (_lockHandleOrderEvent)
            {
                ...
                {
                    Utils.Logger.Error("BrokerageTransactionHandler.HandleOrderEvent(): Unable to locate open Order with id " + orderEvent.OrderId);
                ...
                {
                    Utils.Logger.Error("BrokerageTransactionHandler.HandleOrderEvent(): Unable to resolve open ticket: " + orderEvent.OrderId);
                    LogOrderEvent(orderEvent);
                    return;
                }
                ...
                if (orderEvent.Status == OrderStatus.Filled || orderEvent.Status == OrderStatus.PartiallyFilled)
                {
                        ...
                        && orderEvent.OrderFee.Value.Currency == baseCurrency)
                    {
                        ...
                        if (!_loggedFeeAdjustmentWarning)
                        {
                            ...
                            Utils.Logger.Trace($"BrokerageTransactionHandler.HandleOrderEvent(): {message}");
        ...
        private static void LogOrderEvent(OrderEvent e)
        {
            ...
            {
                Utils.Logger.Debug("BrokerageTransactionHandler.LogOrderEvent(): " + e);
            }
        }
        ...
        private void HandleAccountChanged(AccountEvent account)
        {
            ...
            if (delta != 0)
            {
                Utils.Logger.Trace($"BrokerageTransactionHandler.HandleAccountChanged(): {account.CurrencySymbol} Cash Delta: {delta}");
            }
            ...
            if (_algorithm.Securities.TryGetValue(e.Symbol, out var security))
            {
                ...
                lock (_lockHandleOrderEvent)
                {
                    if (OptionSymbol.IsOptionContractExpired(e.Symbol, CurrentTimeUtc))
                    {
                        ...
                        {
                            Utils.Logger.Error("BrokerageTransactionHandler.HandleOptionNotification(): " +
                    ...
                    else
                    {
                        ...
                        {
                            Utils.Logger.Trace("BrokerageTransactionHandler.HandleOptionNotification(): " +
