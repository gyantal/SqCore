...
namespace QuantConnect.Lean.Engine
{
    ...
    public class Engine
    {
        ...
        public void Run(AlgorithmNodePacket job, AlgorithmManager manager, string assemblyPath, WorkerThread workerThread, bool p_isUseIbFeeModelForEquities = true)
        {
            ...
            try
            {
                Console.WriteLine("***Main8-Run1");
                Utils.Logger.Trace($"Engine.Run(): Resource limits '{job.Controls.CpuAllocation}' CPUs. {job.Controls.RamAllocation} MB RAM.");
                ...
                try
                {
                    ...
                    Console.WriteLine("***Main8-Run5");

                    // set this again now that we've actually added securities
                    // BacktestingResultHandler.SetAlgorithm() hijacks Console.Out. in ConfigureConsoleTextWriter()
                    AlgorithmHandlers.Results.SetAlgorithm(algorithm, AlgorithmHandlers.Setup.StartingPortfolioValue);
                    
                    Console.WriteLine("***Main8-Run5-1"); // doesn't show
                    ...
                    Console.WriteLine("***Main8-Run6"); // doesn't show
                ...
                Console.WriteLine("***Main8-Run7"); // doesn't show
                ...
                Utils.Logger.Trace($"JOB HANDLERS:{Environment.NewLine}" +
                ...
                Console.WriteLine("***Main8-InitComplete");
                ...
                if (initializeComplete)
                {
                    ...
                    {
                        Utils.Logger.Error(err, "Error sending analysis results");
                    }
                    ...
                    && millisecondTotalWait < 30*1000)
                {
                    ...
                    {
                        Utils.Logger.Trace("Waiting for threads to exit...");
                    }
                ...
                if (brokerage != null)
                {
                    Utils.Logger.Trace("Engine.Run(): Disconnecting from brokerage...");
                    brokerage.Disconnect();
                    brokerage.Dispose();
                }
                if (AlgorithmHandlers.Setup != null)
                {
                    Utils.Logger.Trace("Engine.Run(): Disposing of setup handler...");
                    AlgorithmHandlers.Setup.Dispose();
                }

                Utils.Logger.Trace("Engine.Main(): Analysis Completed and Results Posted.");
            }
            catch (Exception err)
            {
                Utils.Logger.Error(err, "Error running algorithm");
            }
            finally
            {
        ...
        private void HandleAlgorithmError(AlgorithmNodePacket job, Exception err)
        {
            ...
            if (AlgorithmHandlers.Results != null)
            {
                ...
                Utils.Logger.Trace("Engine.Run(): Sending runtime error to user...");
