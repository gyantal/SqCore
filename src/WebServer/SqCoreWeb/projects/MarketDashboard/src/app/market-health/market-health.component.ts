...
export class MarketHealthComponent implements OnInit {
  ...
  lookbackStartETstr: string; // set in ctor; We need this for sending String instruction to Server. Anyway, a  HTML <input date> is always a DOMString representing a date in YYYY-MM-DD format, or empty. https://developer.mozilla.org/en-US/docs/Web/HTML/Element/input/date
  lookbackEndET: Date;
  ...
  FillDataWithEmptyValuesToAvoidUiBlinkingWhenDataArrives(): void {
    ...
    for (const tkr of defaultTickerExpected) {
      const sqTicker = tkr === 'BrNAV' ? tkr : 'S/' + tkr;
  ...
  public webSocketOnMessage(msgCode: string, msgObjStr: string): boolean {
    switch (msgCode) {
      case 'MktHlth.NonRtStat':
        ...
        const msgStrNonRt = jsonArrayObjNonRt.map((r) => r.assetId + '|' + r.ticker + '|periodEnd:' + r.periodEnd.toFixed(2).toString() + '|periodStart:' + r.periodStart.toString() + '|open:' + r.periodStart.toFixed(2).toString() + '|high:' + r.periodHigh.toFixed(2).toString() + '|low:' + r.periodLow.toFixed(2).toString() + '|mdd:' + r.periodMaxDD.toFixed(2).toString() + '|mdu:' + r.periodMaxDU.toFixed(2).toString()).join(', ');
  ...
  public webSocketLstValArrived(lstValObj: Nullable<AssetLastJs[]>) {
