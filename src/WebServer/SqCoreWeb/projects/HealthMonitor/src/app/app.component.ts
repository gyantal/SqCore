...
declare let gSqUserEmail: string;
...
export class AppComponent {
  ...
  public getHMData(p_hmDataToSend: HMData): void {
    ...
    this.m_http.post(this.m_baseUrl + '/WebServer/ReportHealthMonitorCurrentStateToDashboardInJSON', p_hmDataToSend).subscribe(result => {
      ...
      const hmData: HMData = result as HMData; // Typescript cast: remember that this is a compile-time cast, and not a runtime cast.
      this.m_data = hmData;
      ...
      const localNow = new Date(); // this is local time: <checked>
      ...
      for (const i in this.m_data.RtpsDownloads) {
      ...
      for (const i in this.m_data.VBrokerReports) {
...
function localDateToUtcString_yyyy_mm_dd_hh_mm_ss(date: Date): string {
  const year = '' + date.getUTCFullYear();
  let month = '' + (date.getUTCMonth() + 1); if (month.length == 1) month = '0' + month;
  let day = '' + date.getUTCDate(); if (day.length == 1) day = '0' + day;
  let hour = '' + date.getUTCHours(); if (hour.length == 1) hour = '0' + hour;
  let minute = '' + date.getUTCMinutes(); if (minute.length == 1) minute = '0' + minute;
  let second = '' + date.getUTCSeconds(); if (second.length == 1) second = '0' + second;
