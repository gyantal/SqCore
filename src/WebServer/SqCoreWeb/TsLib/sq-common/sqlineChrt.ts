...
export function sqLineChartGenerator2(noAssets: number, nCurrData: number, assetNames2Array: string[], isXvalueDate: boolean,
  ...
  if (isXvalueDate) {
    for (let j = 0; j < noAssets; j++) {
      for (let i = 0; i < nCurrData; i++) {
        const chrtData: pctChngStckPriceData = {
          ticker: assetNames2Array[j],
          date: new Date(assChartMtx1[i][0]), // Date()
    ...
    for (let j = 0; j < noAssets; j++) {
      for (let i = 0; i < 7; i++) {
        const chrtData: pctChngStckPriceData = {
          ticker: assetNames2Array[j],
          date: assChartMtx1[i].date, // not Date
          price: parseFloat(assChartMtx1[i].price),
        };
        stckChrtData.push(chrtData);
      }
    }
  ...
  if (isXvalueDate) {
    ...
    xScale = d3.scaleLinear().domain([xMin as number, xMax as number])
        .range([0, width]);
  }
  const yMin = d3.min(stckChrtData, (r:{ price: any; }) => r.price as number);
  const yMax = d3.max(stckChrtData, (r:{ price: any; }) => r.price as number);
  ...
  function onMouseMove(event: any) {
    ...
    if (isXvalueDate) {
      ...
      tooltipPctChg
          ...
          .append('div')
          .style('color', (d: any) => color(d.ticker) as any)
          .html((d: any) => d.ticker + ': ' + yScaleTickFormat + d.priceData.find((h: any) => h.date as any === closestXCoord as any).price);
    }
  }
}