...
export function sqLineChartGenerator1(noAssets: number, nCurrData: number, assetNames2Array: string[], pctChngChrtData, xLabel: string,
  ...
  const xMin = d3.min(pctChngChrtData, (r:{ days: any; }) => r.days as number);
  const xMax = d3.max(pctChngChrtData, (r:{ days: any; }) => r.days as number);
  ...
  lineChrt.selectAll('.line')
      ...
      .attr('d', (d:any) => (d3.line()
          .x((d: any) => xScale(d.days) as number)
  ...
  lineChrt
      ...
      .attr('cx', (d: any) => xScale(d.days) as number)
  ...
  function onMouseMove(event: any) {
    ...
    const closestYCoord = pctChngData[0].priceData.find((h) => h.days === closestXCoord).price;
    ...
    tooltipPctChg
        ...
        .html((d: any) => d.ticker + ': ' + yScaleTickFormat + d.priceData.find((h: any) => h.days === closestXCoord).price + '<br>');
  }
}
