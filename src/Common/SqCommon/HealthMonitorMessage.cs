...
namespace SqCommon
{
    ...
    {
        static bool gSendMsgToHealthMonitorInDebug = false; // in Development, we have many error messages. If it is sent to HealthMonitor app, we receive these unnecessary emails. Better to avoid.
        ...
        public static async Task SendAsync(string p_fullMsg, HealthMonitorMessageID p_healthMonId, TimeSpan? p_globalMinTimeBetweenMessages = null)
        {
            ...
            if ((DateTime.UtcNow - gLastMessageTime) > globalMinTimeBetweenMessages)   // don't send it in every minute, just after e.g. 30 minutes
            {
                ...
                if (Utils.IsDebugRuntimeConfig())
                    sendMessage = gSendMsgToHealthMonitorInDebug;
